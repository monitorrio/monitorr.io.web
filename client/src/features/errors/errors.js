...
Errors.$inject = ['$rootScope', '$compile', 'ENV', 'helpersService'];

function Errors($rootScope, $compile, ENV, helpersService) {
	return {
		restrict: 'E',
		scope: {
			settings: '='
		...
		link: function (scope, elem, attrs) {
			...
			$rootScope.$on('errorTableFilterUpdate', (event, newFilters) => {
				scope.filters = newFilters;
				reloadTable();
			});

			$rootScope.$on('openErrorDetails', (event, errorGuid) => {
			...
			$('#dt-errors').find('tbody').on('click', 'td.details-control', function () {
